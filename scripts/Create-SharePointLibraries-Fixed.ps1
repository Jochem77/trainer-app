...
# Check if SharePoint PnP PowerShell (legacy) is installed for PowerShell 5.1
if (-not (Get-Module -ListAvailable -Name "SharePointPnPPowerShellOnline")) {
    Write-Host "SharePointPnPPowerShellOnline module niet gevonden. Installeren..." -ForegroundColor Yellow
    try {
        Install-Module -Name "SharePointPnPPowerShellOnline" -Force -AllowClobber -Scope CurrentUser
        Write-Host "SharePointPnPPowerShellOnline module succesvol geinstalleerd." -ForegroundColor Green
    }
    catch {
        Write-Error "Kon SharePointPnPPowerShellOnline module niet installeren: $($_.Exception.Message)"
        exit 1
    }
}

# Import SharePoint PnP PowerShell (legacy for PowerShell 5.1)
...
function New-CustomDocumentLibrary {
    ...
    try {
        ...
        # Create the document library (legacy PnP uses different parameters)
        $listUrl = $Title.Replace(" ", "").Replace("-", "")
        $newList = New-PnPList -Title $Title -Template DocumentLibrary -Url $listUrl
        Write-Host "  Documentbibliotheek '$Title' succesvol aangemaakt." -ForegroundColor Green
        
        # Set description separately for legacy module
        if ($Description) {
            Set-PnPList -Identity $Title -Description $Description
        }
        
        # Apply command bar formatting (may not work with legacy module)
        try {
            Write-Host "  Toepassen van command bar formatting..." -ForegroundColor Cyan
            # Legacy module may not support this feature
            # Set-PnPView -List $Title -Identity "All Documents" -Values @{ "CustomFormatter" = $JsonFormat }
            Write-Host "  Command bar formatting overgeslagen (legacy module beperking)." -ForegroundColor Yellow
        }
        catch {
            Write-Host "  Command bar formatting niet ondersteund in legacy module." -ForegroundColor Yellow
        }
        
        # Add to Quick Launch (left navigation)
        Write-Host "  Toevoegen aan Quick Launch navigatie..." -ForegroundColor Cyan
        try {
            $navUrl = "$($SiteUrl)/$listUrl"
            Add-PnPNavigationNode -Location QuickLaunch -Title $Title -Url $navUrl
        }
        catch {
            Write-Host "  Fout bij toevoegen aan navigatie: $($_.Exception.Message)" -ForegroundColor Yellow
        }
...
try {
    ...
    if ($successCount -gt 0) {
        Write-Host ""
        Write-Host "De volgende documentbibliotheken zijn aangemaakt:" -ForegroundColor Green
        foreach ($werkmap in $Werkmappen) {
            Write-Host "  - $($werkmap.Title)" -ForegroundColor White
        }
        
        Write-Host ""
        Write-Host "Elke bibliotheek heeft:" -ForegroundColor Yellow
        Write-Host "  checkmark Custom command bar formatting" -ForegroundColor White
        Write-Host "  checkmark Snelkoppeling in de linkernavigatie" -ForegroundColor White
        Write-Host "  checkmark Aangepaste beschrijving" -ForegroundColor White
    }
    ...
    try {
        ...
        Write-Host ""
        Write-Host "Verbinding met SharePoint gesloten." -ForegroundColor Yellow
    }
    catch {
...
Write-Host ""
Write-Host "Script voltooid. Druk op een toets om te sluiten..." -ForegroundColor Cyan
